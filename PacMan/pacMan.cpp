/* g++ pacMan.cpp -o pacMan `allegro-config --static` */
...
PACKFILE *pack_fopen(const char *filename, const char *mode);
 ...
 PACKFILE *input_file;
          ...
          input_file = pack_fopen("mapapacman.txt", "rw");
          if (!input_file)
             abort_on_error("no puedo leer 'mapapacman.txt'!"); 
